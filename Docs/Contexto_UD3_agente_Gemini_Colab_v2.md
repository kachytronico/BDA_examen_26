...
## UD3 · Ecosistema Hadoop · contexto operativo integrado
...
- y no cambie el sentido de los apartados por comodidad.
...
### No casarse con una única vía de importación
...
- nombre de archivos,
- cabeceras,
- claves (`id`, `user_id`, `client_id`, `card_id`, etc.),
- presencia de fechas, importes, nulos o vacíos.
...
Debe:
- descargar, copiar o localizar archivos,
- descomprimir si hace falta,
- y verificar la carpeta o ruta final.
...
Debe:
- verificar versión y entorno,
- crear `SparkSession`,
- activar soporte Hive si va a guardar tablas.
...
Debe:
- crear base si hace falta,
- usar nombres claros,
- guardar con `saveAsTable`,
...
Debe hacer al menos:
- una consulta simple útil,
- y una consulta con `JOIN` entre 2 tablas.

Si el enunciado lo exige, debe asegurar 2 consultas con 2 tablas.
...
- tablas creadas,
- tipos finales,
- y que el join devuelve algo coherente.
...
## 7. Qué cambiar y qué no cambiar
### Cambia siempre
- rutas,
- nombres de archivo,
- nombres de columnas,
...
## 8. Regla de adaptación por herramienta
...
#### Regla
No confiar ciegamente en `inferSchema`.
...
## 9. Núcleos mínimos para principiante
### Pandas
- `read_csv`
- `head`
- `info`
...
## 10. Traducción rápida de cada fase
...
## 11. Errores frecuentes que el agente debe vigilar
...
### Pandas
- columna numérica como texto,
- separador mal detectado,
- no revisar nulls ni tipos.
...
- leer otro archivo distinto al perfilado antes.

### Hive
- `saveAsTable` falla por catálogo o por no usar la base correcta,
- tablas vacías,
- `SHOW TABLES` no coincide con lo que creías guardar.

### HQL
- join vacío,
- columnas ambiguas,
- claves que no tienen el mismo significado,
- agregar importes sin cast ni limpieza.

### Copiar sin adaptar
- el código corre pero no responde al dataset real,
- columnas inexistentes,
- joins absurdos,
- rutas heredadas de otra tarea.

---

## 12. Qué debe comprobar antes de cada bloque
...
- si las tablas que guarda son las mismas que luego consulta,
- y si las claves de join son realmente válidas.

---

## 13. Qué no puede hacer el agente en UD3
### Prohibido
...
## 14. Señales de que UD3 está bien resuelto
...
## 15. Señales de que el agente se está equivocando
...
- no revisa schemas,
- trata una entrada concreta como universal,
...
Si el agente rompe esa cadena sin motivo, hay que corregir el punto.
