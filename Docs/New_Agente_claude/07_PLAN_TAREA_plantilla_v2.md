...
> - Añadido campo "Archivos a subir a HDFS (UD2/UD3)" para que el agente liste **todos** los archivos que entran al pipeline Hadoop, no solo el dataset principal.
...
> Solo aplica si el examen tiene apartados de UD2 o UD3. Listar **todos** los archivos físicos que el pipeline Hadoop debe procesar, con su ruta de origen y el nombre corto que tendrán en `/content/` y en la carpeta HDFS `dataset/`.
...
**Nota:** todos los archivos van a la misma carpeta HDFS `dataset/` y los jobs Hadoop (grep, Java, Streaming) procesan la carpeta entera, no archivos sueltos.
