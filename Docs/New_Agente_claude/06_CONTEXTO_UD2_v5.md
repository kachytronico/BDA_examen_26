...
> **Versión:** v5 (alineada con el sistema BDA v9 — plantillas operativas inyectadas por chat)
...
> **Cambio respecto a v4:** las plantillas operativas (Instalación, HDFS, Java, Streaming) **ya no están pre-cargadas en el cuaderno**. Existen como archivos `.md` independientes y el agente las pide al usuario por chat al llegar al apartado correspondiente. Esto reduce ruido en el cuaderno y permite escalar el sistema a UD3 sin saturar el contexto.
...
UD2 tiene **cuatro plantillas operativas** disponibles como archivos `.md`. **No están pre-cargadas en el cuaderno**: el usuario las inyecta por chat cuando el agente las pide.

| Nombre del archivo | Para qué apartado | Qué contiene |
|---|---|---|
| `plantilla_01_instalacion_hadoop.md` | Antes de empezar UD2 | Instala Hadoop 3.4.2 y configura `JAVA_HOME`, `HADOOP_HOME`, `PATH` |
| `plantilla_02_hdfs.md` | Apartado "Operaciones HDFS" | Patrón de 4 pasos: `shutil.copy` a `/content/` → `mkdir dataset` → `put -f` → `ls` |
| `plantilla_03_java_wordcount.md` | Apartado "MapReduce Java" | Tres archivos separados (`WordCount.java`, `WordCountMapper.java`, `WordCountReducer.java`) + celda de compilación y ejecución |
| `plantilla_04_streaming.md` | Apartado "MapReduce Streaming" | `mapper.py` + `reducer.py` + celda de ejecución con `hadoop-streaming-*.jar` |

### Cómo trabaja el agente con las plantillas

Al llegar a un apartado de UD2:

1. **Identifica** qué plantilla corresponde (ver columna "Para qué apartado").
2. **Comprueba** si la plantilla ya se la pasaron por chat en esta sesión.
3. **Si no la tiene, la pide al usuario explícitamente** antes de proponer el bloque:
   > *"Para este apartado necesito la `plantilla_03_java_wordcount.md`. ¿Me la pasas por chat?"*
4. **Una vez recibida**, la usa según las reglas de adaptación (ver más abajo). Si una plantilla anterior ya se inyectó al cuaderno (por ejemplo Hadoop ya está instalado), no la pide de nuevo.
...
- **Bloques de comandos directos** (Python, Bash con `!hdfs dfs`, `!hadoop jar`, etc.): el agente crea celdas adaptando los `[ADAPTAR]` con valores de PLAN_TAREA.
...
- **No regenerar.** Si el usuario pasa una plantilla y el agente cree que está incompleta o tiene un error, **avisa al usuario y se detiene**. No la rehace en silencio.
- **No duplicar.** Si una plantilla ya se inyectó al cuaderno (existe la celda con su contenido), el agente NO la vuelve a escribir. Si necesita modificarla, modifica la celda existente o crea una celda secundaria con la adaptación.
...
4. **plantillas operativas** (sección 0, inyectadas por chat al llegar al apartado),
...
**Plantilla operativa requerida:** `plantilla_02_hdfs.md` (también requiere que `plantilla_01_instalacion_hadoop.md` se haya inyectado antes).
...
**Plantilla operativa requerida:** `plantilla_03_java_wordcount.md`.

**Pipeline obligatorio:**
1. pedir la plantilla por chat si no se tiene,
...
**Plantilla operativa requerida:** `plantilla_04_streaming.md`.
...
### 7.4 MapReduce Java (asumiendo plantilla 03 inyectada por chat)
```bash
...
### 7.5 Hadoop Streaming Python (asumiendo plantilla 04 inyectada por chat)
```bash
# Limpieza preventiva
...
- Genera código Java sin haber pedido la plantilla por chat (la inventa desde cero).
